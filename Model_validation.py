...
from models.BeTrsDRP_mut import BeTrsDRP_mut
...
def predicting(model, device, loader):
    ...
    preds.to_csv("data/data_pred/preds.csv")

    labels = np.array(total_labels.cpu().detach().numpy().flatten(), dtype=np.float32)
    labels = pd.DataFrame(labels)
    labels.to_csv("data/data_pred/labels.csv")

    return total_labels.numpy().flatten(),total_preds.numpy().flatten()

model = BeTrsDRP_mut()
model.load_state_dict(torch.load('log/model/model_BeTrsDRP_mut_GDSC.model'))
model.eval()
...
plt.ylabel('RMSE')
plt.title('GE & METH')
plt.savefig("log/Blind_rmse.png", bbox_inches='tight')
...
plt.ylabel('CCp')
plt.title('GE & METH')
plt.savefig("log/Blind_ccp.png", bbox_inches='tight')